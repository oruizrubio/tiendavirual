...
<?php
	if (isset($_SESSION['k_username'])) {		
		...
		if (!isset($accion)) {
			$result = Soul::getInstance()->consultar_producto($codigo);			
			$row=mysqli_fetch_array($result);
			
			echo "
				<h3> Actualizar artículo: </h3>
				<form method=\"POST\" action=\"actualizar_producto.php?accion=guardar\">
				<p> Código: <br> <input type=\"text\" name=\"codigo\" value='".$row[0]."'> </p>
				<p> Nombre: <br> <input type=\"text\" name=\"nombre\" value='".$row[1]."'> </p>
				<p> Precio: <br> <input type=\"text\" name=\"precio\" value='".$row[2]."'> </p>
				<p> En stock: <br> <input type=\"text\" name=\"stock\" value='".$row[3]."'> </p>
				<p> <input type=\"submit\" name=\"submit\" value=\"Guardar\"> </p>
				</form>";					
		} elseif ($accion=="guardar") {
			Soul::getInstance()->actualizar_producto($_POST["codigo"], $_POST["nombre"], $_POST["precio"], $_POST["stock"]);
			$_SESSION["actualizado"] = "S";							
