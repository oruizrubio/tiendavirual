...
<?php
	...
	if (isset($_SESSION['k_username'])) {
		...
		<tbody>";
		$cdi_pedido = $_GET['cdi_pedido'];
		$result = Soul::getInstance()->consultar_pedido($cdi_pedido);
		if ($result != null) {
			while ($row = mysqli_fetch_array($result)){
			echo "<tr>
				<td>$row[0]</td><td>$row[2]</td><td>$row[1]</td>
				</tr>";
			}	
		}
		echo "</tbody></table>";
		...
		<tbody>";
		$result = Soul::getInstance()->consultar_productos_pedido($cdi_pedido);
		if ($result != null) {
			while ($row = mysqli_fetch_array($result)){
			echo "<tr>
		...
		echo "</tbody></table>";
		echo "<p><a class=\"btn\" href=\"pedidos.php\">Volver &raquo;</a></p>";
	}
	else {
