          ...
          <div class="nav-collapse collapse">
            <ul class="nav">
              ...
              <li><?php if ($_SESSION["k_administrador"]=='S') {
				  ...
				  <?php } ?></a></li>
              <li class="active"><a href="cesta.php">Cesta</a></li>
...
<?php
	...
	if (isset($_SESSION['k_username'])) {
		...
		<tbody>";
		$result = Soul::getInstance()->consultar_cesta();
		if ($result != null) {
			while ($row = mysqli_fetch_array($result)){
			echo "<tr>
				<td><a href=\"\"> $row[0]</a></td><td>$row[2]</td><td>$row[1]</td>
				</tr>";
			}	
		}
		echo "</tbody></table>";
		...
		<thead>
		<tr>
			<th>Nombre</th><th>Cantidad</th><th>Precio</th>
		</tr>
		</thead>
		<tbody>";
		$result = Soul::getInstance()->consultar_productos_cesta();
		if ($result != null) {
			while ($row = mysqli_fetch_array($result)){
			echo "<tr>
				<td><a href=\"\"> $row[2]</a></td><td>$row[0]</td><td>$row[1]</td>
				</tr>";
			}	
		}
		echo "</tbody></table>";
		if ($result==null) {
			echo "No hay artículos en la cesta";
		} else {
			echo "</p><p><a href=\"confirmar_cesta.php\" class=\"btn btn-primary btn-large\">Validar cesta</a></p>";			
		}
	}
	else {
